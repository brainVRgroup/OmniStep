...
public class KATXRWalker : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        rotation = ws.bodyRotationRaw * Quaternion.Inverse( Quaternion.Euler(new Vector3(0,yawCorrection,0)));
            ...
            case ExecuteMethod.CharactorController: 
                {
                    ...
                    ch.SimpleMove(rotation * ws.moveSpeed);
                }
                break;
            case ExecuteMethod.MovePosition:
                {
                    transform.position += (rotation * ws.moveSpeed * Time.fixedDeltaTime);
                }
                break;
            case ExecuteMethod.RigidBody:
                {
                    var r = GetComponent<Rigidbody>();
                    r.velocity = rotation * ws.moveSpeed;
                }
                break;
        } 
    }
